...
internal static class TuflowRunHelper
{
    ...
    /// <summary>
    /// Path to TuflowLauncher.exe on this machine. Override via the HEIMDALL_TUFLOW_LAUNCHER_EXE
    /// environment variable (or hardcode a deployment-specific default here) — there's no existing
    /// Heimdall config surface for agent-local tool paths, so this doesn't try to invent one.
    /// </summary>
    private static string LauncherExePath =>
        Environment.GetEnvironmentVariable("HEIMDALL_TUFLOW_LAUNCHER_EXE")
        ?? @"C:\Heimdall\TuflowLauncher\TuflowLauncher.exe";
